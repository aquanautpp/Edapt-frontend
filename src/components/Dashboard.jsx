            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              {aiInsights.recommendedNextSteps.map((step, index) => (
                <div key={index} className="flex items-center justify-between py-2">
                  <span>{step}</span>
                  <Button onClick={() => {
                    if (step.includes('álgebra')) onNavigate('algebra');
                    else if (step.includes('interpretação de texto')) onNavigate('text-interpretation');
                    else if (step.includes('frações')) onNavigate('fractions');
                  }}>
                    Começar
                  </Button>
                </div>
